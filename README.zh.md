<p align="center">
  <a href="README.zh.md">中文</a> | <a href="README.es.md">Español</a> | <a href="README.fr.md">Français</a> | <a href="README.hi.md">हिन्दी</a> | <a href="README.it.md">Italiano</a> | <a href="README.ja.md">日本語</a> | <a href="README.pt-BR.md">Português (BR)</a> | <a href="README.md">English</a>
</p>
...
代码仓库签名决策系统。它会针对任何代码仓库运行新鲜度检查，并输出一个结构化的决策包，包含层级、格式、约束、钩子以及带有 `file:line` 引用信息的“事实”数据。
...
| `verify [repo-path] --artifact <path>` | 对代码仓库中的组件进行检查，与蓝图和事实数据包进行比较 |
| `review [repo-path]` | 打印一份包含 4 个部分的编辑审查报告 |
| `catalog` | 生成季度目录 |
...
| `org status` | 覆盖率、多样性评分、缺失项 |
| `org ledger [n]` | 最近 N 个决策 |
| `org bans` | 当前自动阻止列表及其原因 |
| `config get [key]` | 读取配置值 |
| `config set <key> <value>` | 设置配置（例如，`agent_name`） |
...
将决策包写入到目标代码仓库的 `.artifact/decision_packet.json` 文件中：
...
- **没有遥测。** 除了连接到本地 Ollama 之外，没有网络调用。
- **没有密钥。** 不读取、存储或传输凭据。
- **历史记录在本地。** `.artifact/` 目录位于代码仓库中，默认情况下被 git 忽略。
- **备用方案是确定性的。** 如果 Ollama 无法使用，则输出将基于代码仓库信号进行种子生成，可重现，而不是随机。
- **文件范围：** 读取代码仓库中的源代码文件，仅写入 `.artifact/` 目录和 `~/.artifact/` 目录。

## 环境变量

| 变量 | 用途 |
|----------|---------|
| `OLLAMA_HOST` | 覆盖 Ollama 端点（默认：自动检测） |
